 ...
 * 136 ViewEvidence record(s), keyed by name.
 */
import type { ViewEvidence } from '../evidence-types';

export const coreViews: Record<string, ViewEvidence> = {
  "H_AAA_EntityProfile": {
  ...
  "Vi_BKC_AlivenessSessionZoneQueueEntrance": {
    "name": "Vi_BKC_AlivenessSessionZoneQueueEntrance",
    "source": "core",
    "columns": [
      "AlivenessSessionId",
      "CurrentOrderInQueue",
    ...
    "definition": "select `q`.`AlivenessSessionId` AS `AlivenessSessionId`,`q`.`CurrentOrderInQueue` AS `CurrentOrderInQueue`,greatest(coalesce(`a`.`ModifiedAt_UTC`,'2000-01-01'),coalesce(`q`.`AtQueueZoneSince_UTC`,'2000-01-01')) AS `EnteredQueueAt_UTC`,`q`.`EffectiveQueueZoneId` AS `EffectiveQueueZoneId`,(case when (`q`.`PriorityEnd_UTC` > utc_timestamp()) then `q`.`QueuePriority` else 10 end) AS `EffectiveQueuePriority`,`a`.`Status` AS `Status` from (`_AlivenessSessionQueueStatus` `q` left join `_AlivenessSessionAvailabilityStatus` `a` on((`a`.`AlivenessSessionId` = `q`.`AlivenessSessionId`))) where (`a`.`Status` = 1)"
  },
  "Vi_BKC_DriverStatus": {
    "name": "Vi_BKC_DriverStatus",
    "source": "core",
    "columns": [
      "DriverId",
      "ActivitySessionId",
    ...
    "definition": "select `act`.`DriverId` AS `DriverId`,`act`.`Id` AS `ActivitySessionId`,`asl`.`Timestamp_UTC` AS `StatusTimestamp_UTC`,`asl`.`Status` AS `ActivityStatus` from (`_ActivityStatusLog` `asl` join `_ActivitySessions` `act` on((`act`.`Id` = `asl`.`ActivitySessionId`)))"
  },
  "Vi_BKC_LiveUserWsSessions": {
    "name": "Vi_BKC_LiveUserWsSessions",
    "source": "core",
    "columns": [
      "Id",
      "ClientType",
  ...
  "Vi_BKC_PermitStatusApprovals": {
    ...
    "columns": [
      ...
      "RequestApproverEntityId",
      "RequestApproverActENUM",
      "ApproverEntityId",
      "ApproverActENUM",
      "Id",
      "PermitId",
  ...
  "Vi_BKC_QueueSnapshot": {
    "name": "Vi_BKC_QueueSnapshot",
    "source": "core",
    "columns": [
      "ActivitySessionId",
      "AlivenessSessionId",
      ...
      "LocationSubZoneCode",
      "EnteredLocationZoneAt_UTC",
      "EffectiveQueueZoneId",
      "AtQueueZoneSince_UTC",
      "CurrentOrderInQueue",
      "QueuePriority",
      "PriorityStart_UTC",
      "PriorityEnd_UTC",
      "AvailabilityStatus",
      "ComputedOrderInQueue",
      "Id",
      "ActivitySessionId",
      "MaxAlivenessId"
    ],
    "definition": "select `ac`.`Id` AS `ActivitySessionId`,`latest_as`.`Id` AS `AlivenessSessionId`,`ac`.`ServiceCategory` AS `ServiceCategory`,`ac`.`ParticipationCategory` AS `ParticipationCategory`,`ac`.`DriverId` AS `DriverId`,`ac`.`VehicleId` AS `VehicleId`,`asq`.`CurrentLocationZoneId` AS `CurrentLocationZoneId`,`asq`.`LocationSubZoneCode` AS `LocationSubZoneCode`,`asq`.`EnteredLocationZoneAt_UTC` AS `EnteredLocationZoneAt_UTC`,`asq`.`EffectiveQueueZoneId` AS `EffectiveQueueZoneId`,`asq`.`AtQueueZoneSince_UTC` AS `AtQueueZoneSince_UTC`,`asq`.`CurrentOrderInQueue` AS `CurrentOrderInQueue`,`asq`.`QueuePriority` AS `QueuePriority`,`asq`.`PriorityStart_UTC` AS `PriorityStart_UTC`,`asq`.`PriorityEnd_UTC` AS `PriorityEnd_UTC`,`asav`.`Status` AS `AvailabilityStatus`,row_number() OVER (PARTITION BY `ac`.`ServiceCategory`,`ac`.`ParticipationCategory`,`asq`.`EffectiveQueueZoneId` ORDER BY (case when ((`asq`.`QueuePriority` between 1 and 9) and (`asq`.`PriorityEnd_UTC` is not null) and (`asq`.`PriorityEnd_UTC` > utc_timestamp())) then `asq`.`QueuePriority` else 10 end),`asq`.`AtQueueZoneSince_UTC`,`asq`.`AlivenessSessionId` )  AS `ComputedOrderInQueue` from (((`_ActivitySessions` `ac` join (select `alv`.`Id` AS `Id`,`alv`.`ActivitySessionId` AS `ActivitySessionId`,max(`alv`.`Id`) OVER (PARTITION BY `alv`.`ActivitySessionId` )  AS `MaxAlivenessId` from `_AlivenessSessions` `alv` where ((`alv`.`CurrentStatus` <> 4) and (`alv`.`EndAt_UTC` is null))) `latest_as` on(((`latest_as`.`ActivitySessionId` = `ac`.`Id`) and (`latest_as`.`Id` = `latest_as`.`MaxAlivenessId`)))) join `_AlivenessSessionQueueStatus` `asq` on((`asq`.`AlivenessSessionId` = `latest_as`.`Id`))) join `_AlivenessSessionAvailabilityStatus` `asav` on((`asav`.`AlivenessSessionId` = `latest_as`.`Id`))) where ((`ac`.`EndAt_UTC` is null) and (`asav`.`Status` in (1,8,9)) and (`asq`.`EffectiveQueueZoneId` is not null) and (`asq`.`AtQueueZoneSince_UTC` is not null))"
  },
  "Vi_BKC_TransportFareCalculatorPrettier": {
    "name": "Vi_BKC_TransportFareCalculatorPrettier",
    "source": "core",
    "columns": [
      "Id",
      "DistributorId",
  ...
  "Vi_BKC_VehicleStatus": {
    ...
    "columns": [
      ...
      "AlivenessSessionId",
      "CurrentAlivenessStatus",
      "CurrentAlivenessStatusReason",
      "LastLongitude",
      "LastLatitude",
      "LastZonePuls_UTC",
      "LastPosiPuls_UTC",
  ...
  "Vi_DPA_ActorConfigsDetail": {
    "name": "Vi_DPA_ActorConfigsDetail",
    "source": "core",
    "columns": [
      "ActionConfigBaseId",
    ...
    "definition": "select `BaseTable`.`Id` AS `ActionConfigBaseId`,`BaseTable`.`Code` AS `ActionConfigBaseCode`,`BaseTable`.`Description` AS `ActionConfigBaseDescription`,`RJT`.`EntityId` AS `EntityId`,`RJT`.`VehicleId` AS `VehicleId`,`RJT`.`UserId` AS `UserId`,`RJT`.`ActENUM` AS `ActENUM`,`RJT`.`ScenarioId` AS `ScenarioId`,`RJT`.`Value` AS `Value`,`RJT`.`IsOverridable` AS `IsOverridable` from (`H_CFG_ActionConfigBases` `BaseTable` join `H_CFG_ActorConfigs` `RJT` on(((`RJT`.`ConfigBaseId` = `BaseTable`.`Id`) and ((0 <> `RJT`.`RecordDeleted`) is not true)))) where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_AppClient": {
  ...
  "Vi_DPA_CVOServicesSuperDetail": {
    ...
    "definition": "select `BaseTable`.`Id` AS `CVOId`,`BaseTable`.`EntityId` AS `CVOEntityId`,`AP`.`Title` AS `CVOEntityTitle`,`AP`.`Address_fsx` AS `CVOEntityAddress_fsx`,`AP`.`City` AS `CVOEntityCity`,`AP`.`Province` AS `CVOEntityProvince`,`AP`.`Country` AS `CVOEntityCountry`,`AP`.`PostalCode_fsx` AS `CVOEntityPostalCode_fsx`,`AP`.`Phone_fsx` AS `CVOEntityPhone_fsx`,`AP`.`FaxNumber_fsx` AS `CVOEntityFaxNumber_fsx`,`AP`.`Email_fsx` AS `CVOEntityEmail_fsx`,`AP`.`Discriminator` AS `CVOEntityDiscriminator`,`RJT`.`ServiceId` AS `ServiceId`,`RJT`.`ServiceCategory` AS `ServiceCategory`,`PJT0`.`ServiceType` AS `ServiceBaseServiceType`,`PJT0`.`ServiceClass` AS `ServiceBaseServiceClass` from (((`H_Synced_CVOs` `BaseTable` join `H_AAA_Synced_EntityProfile` `AP` on(((`BaseTable`.`EntityId` = `AP`.`Id`) and ((0 <> `AP`.`RecordDeleted`) is not true)))) join `H_Synced_CVOServices` `RJT` on(((`RJT`.`CVOId` = `BaseTable`.`Id`) and ((0 <> `RJT`.`RecordDeleted`) is not true)))) join `H_Synced_ServiceBases` `PJT0` on(((`RJT`.`ServiceId` = `PJT0`.`Id`) and ((0 <> `PJT0`.`RecordDeleted`) is not true)))) where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  ...
  "Vi_DPA_ClientDeviceInfo": {
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`ClientOS` AS `ClientOS`,`BaseTable`.`DeviceModel` AS `DeviceModel`,`BaseTable`.`AppID` AS `AppID`,`BaseTable`.`VehicleQRCode` AS `VehicleQRCode`,`BaseTable`.`AppVersion` AS `AppVersion` from `_ClientDeviceInfo` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_DelegatedDispatchContract": {
    "name": "Vi_DPA_DelegatedDispatchContract",
    "source": "core",
    "columns": [
      "Id",
      "RequestId",
      ...
      "PolicyVersionRef",
      "ConfigVersionRef",
      "OperationModeSnapshot",
      "DelegateServiceIdentity",
      "ActivatedAt_UTC",
      "AcceptedAt_UTC",
      "ReclaimedAt_UTC",
      "ReclaimReason",
  ...
  "Vi_DPA_DispatchRuleSet": {
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`Name` AS `Name`,`BaseTable`.`PolicyFamily` AS `PolicyFamily`,`BaseTable`.`OwnerScopeType` AS `OwnerScopeType`,`BaseTable`.`OwnerScopeId` AS `OwnerScopeId`,`BaseTable`.`Status` AS `Status` from `_DispatchRuleSets` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_DispatchRuleSetSummary": {
    "name": "Vi_DPA_DispatchRuleSetSummary",
    "source": "core",
    "columns": [
      "Id",
      "Name",
  ...
  "Vi_DPA_DispatchRuleVersion": {
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`RuleSetId` AS `RuleSetId`,`BaseTable`.`VersionNo` AS `VersionNo`,`BaseTable`.`LifecycleState` AS `LifecycleState`,`BaseTable`.`SourceHash` AS `SourceHash`,`BaseTable`.`CompiledArtifactId` AS `CompiledArtifactId`,`BaseTable`.`ValidationResult` AS `ValidationResult`,`BaseTable`.`ActivatedAt_UTC` AS `ActivatedAt_UTC` from `_DispatchRuleVersions` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_DispatchRuleVersionSummary": {
    "name": "Vi_DPA_DispatchRuleVersionSummary",
    "source": "core",
    "columns": [
      "Id",
      "RuleSetId",
  ...
  "Vi_DPA_DriversSuperDetail": {
    "name": "Vi_DPA_DriversSuperDetail",
    "source": "core",
    "columns": [
      "CVOId",
      "CVOEntityId",
  ...
  "Vi_DPA_EntityScenarioPermitsExtraDetail": {
    "name": "Vi_DPA_EntityScenarioPermitsExtraDetail",
    "source": "core",
    "columns": [
      "EntityScenarioId",
      "EntityScenarioEntityId",
  ...
  "Vi_DPA_Feature": {
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`Title` AS `Title` from `H_Synced_Features` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  ...
  "Vi_DPA_RollCallSession": {
    "name": "Vi_DPA_RollCallSession",
    "source": "core",
    "columns": [
      "Id",
      ...
      "ModeType",
      "TopXPerZone",
      "PrimaryGroupSnapshot",
      "StartedAt_UTC",
      "ClosedAt_UTC"
    ],
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`DelegatedDispatchContractId` AS `DelegatedDispatchContractId`,`BaseTable`.`SessionStatus` AS `SessionStatus`,`BaseTable`.`ScopeType` AS `ScopeType`,`BaseTable`.`ScopeId` AS `ScopeId`,`BaseTable`.`WindowStart_UTC` AS `WindowStart_UTC`,`BaseTable`.`WindowEnd_UTC` AS `WindowEnd_UTC`,`BaseTable`.`SignupClosesAt_UTC` AS `SignupClosesAt_UTC`,`BaseTable`.`ModeType` AS `ModeType`,`BaseTable`.`TopXPerZone` AS `TopXPerZone`,`BaseTable`.`PrimaryGroupSnapshot` AS `PrimaryGroupSnapshot`,`BaseTable`.`StartedAt_UTC` AS `StartedAt_UTC`,`BaseTable`.`ClosedAt_UTC` AS `ClosedAt_UTC` from `_RollCallSessions` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_RollCallSessionParticipantsDetail": {
    "name": "Vi_DPA_RollCallSessionParticipantsDetail",
    "source": "core",
    "columns": [
      "RollCallSessionId",
      "RollCallSessionDelegatedDispatchContractId",
      "RollCallSessionSessionStatus",
      "RollCallSessionScopeType",
      "RollCallSessionScopeId",
      "RollCallSessionWindowStart_UTC",
      "RollCallSessionWindowEnd_UTC",
      "RollCallSessionSignupClosesAt_UTC",
      "RollCallSessionModeType",
      "RollCallSessionTopXPerZone",
      "RollCallSessionPrimaryGroupSnapshot",
      "RollCallSessionStartedAt_UTC",
      "RollCallSessionClosedAt_UTC",
      "ParticipantId",
      "ActivitySessionId",
      "PriorityGroupId",
      "SignupAt_UTC",
  ...
  "Vi_DPA_ServiceBase": {
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`ServiceType` AS `ServiceType`,`BaseTable`.`ServiceClass` AS `ServiceClass` from `H_Synced_ServiceBases` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  ...
  "Vi_DPA_TransportBatch": {
    "name": "Vi_DPA_TransportBatch",
    "source": "core",
    "columns": [
      "Id",
      "FMS_OId",
  ...
  "Vi_DPA_TransportContactLocation": {
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`ContactId` AS `ContactId`,`BaseTable`.`FavAdrLabel` AS `FavAdrLabel`,`BaseTable`.`LocationId` AS `LocationId` from `H_TransportContactLocations` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_TransportContactLocationSummary": {
    "name": "Vi_DPA_TransportContactLocationSummary",
    "source": "core",
    "columns": [
      "Id",
      "ContactId",
  ...
  "Vi_DPA_TransportContactRegistriesSuperDetail": {
    ...
    "columns": [
      ...
      "PhotoUri",
      "BirthDate_fsx",
      "Notification_SMS_fsx",
      "Notification_Phone_fsx",
      "Notification_Phone_Extention_fsx",
      "Notification_Email_fsx",
      "Notification_WhatsApp_fsx",
      "UserInfoFirstname_fsx",
      "UserInfoLastname_fsx",
      "UserInfoUsername",
      "UserInfoEmail_fsx",
  ...
  "Vi_DPA_TransportContactRegistrySummary": {
    ...
    "columns": [
      ...
      "UserCellPhoneConfirmed",
      "UserIssuedBy",
      "TransportContactLocationsCount",
      "TransportCustomerContactsCount",
      "ContactId",
      "RelatedEntityCount",
      "ContactId",
      "RelatedEntityCount"
  ...
  "Vi_DPA_TransportCustomerContactsSuperDetail": {
    "name": "Vi_DPA_TransportCustomerContactsSuperDetail",
    "source": "core",
    "columns": [
      "TransportContactRegistryId",
      "TransportContactRegistryPossibleUserId",
      ...
      "TransportContactRegistryUserIssuedBy",
      "CustomerId",
      "TransportCustomerEntityId",
      "TransportCustomerUserId",
      "TransportCustomerFirstname_fsx",
      "TransportCustomerLastname_fsx",
      "TransportCustomerUsername",
      "TransportCustomerEmail_fsx",
      "TransportCustomerEmail_hash",
      "TransportCustomerEmailConfirmed",
      "TransportCustomerCellPhone_fsx",
      "TransportCustomerCellPhone_hash",
      "TransportCustomerCellPhoneConfirmed",
      "TransportCustomerIssuedBy",
      "TransportCustomerRecordKey",
      "TransportCustomerCreatedAt_UTC",
      "TransportCustomerCreatedBy",
      "TransportCustomerModifiedAt_UTC",
      "TransportCustomerModifiedBy",
      "TransportCustomerRecordDeleted"
    ],
  ...
  "Vi_DPA_TransportFareBaseInclutionsSuperDetail": {
    "name": "Vi_DPA_TransportFareBaseInclutionsSuperDetail",
    "source": "core",
    "columns": [
      "DistributorId",
      "DistributorEntityId",
      ...
      "ServiceClass",
      "IncludedSeatCounts",
      "IncludedLuggageCounts",
      "IncludedParcelCounts",
      "IncludedWeightKg",
      "IncludedVolumeCubicM",
      "CVOEntityId",
      "CVOTitle",
      "CVOAddress_fsx",
      "CVOCity",
  ...
  "Vi_DPA_TransportFareCalculation": {
    "name": "Vi_DPA_TransportFareCalculation",
    "source": "core",
    "columns": [
      "Id",
      "DistributorId",
  ...
  "Vi_DPA_TransportGeoFenceBase": {
    "name": "Vi_DPA_TransportGeoFenceBase",
    "source": "core",
    "columns": [
      "Id",
      "Code",
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`Code` AS `Code`,`BaseTable`.`Title` AS `Title`,`BaseTable`.`Coordinates` AS `Coordinates`,`BaseTable`.`Order` AS `Order` from `H_TransportGeoFenceBases` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_TransportLocation": {
    "name": "Vi_DPA_TransportLocation",
    "source": "core",
    "columns": [
      "Id",
      "Title",
  ...
  "Vi_DPA_TransportReqDeliveriesSuperDetail": {
    "name": "Vi_DPA_TransportReqDeliveriesSuperDetail",
    "source": "core",
    "columns": [
      "DistributorId",
      "DistributorEntityId",
      ...
      "ToContactLocationId",
      "PickupAfterDateTime",
      "PickupAfterDateTime_UTC",
      "PickupBeforeDateTime",
      "PickupBeforeDateTime_UTC",
      "DropOffAfterDateTime",
      "DropOffAfterDateTime_UTC",
      "DropOffBeforeDateTime",
      "DropOffBeforeDateTime_UTC",
      "VehicleType",
      "VehicleProfileSubType",
      "ServiceClass",
      "ActionOnAbsentReceiver",
      "Comments",
      "FareAtSubmission",
      "PromoCode",
      "PaymentCategory",
      ...
      "PaidAt_UTC",
      "Status",
      "TransportContactLocationContactId",
      "TransportContactLocationFavAdrLabel",
      "TransportContactLocationLocationId"
    ],
  ...
  "Vi_DPA_TransportReqDelivery": {
    "name": "Vi_DPA_TransportReqDelivery",
    "source": "core",
    "columns": [
      "Id",
      "IsEditable",
  ...
  "Vi_DPA_TransportReqDeliveryGoodsSuperDetail": {
    ...
    "columns": [
      ...
      "TransportReqDeliveryStatus",
      "CustomerGoodId",
      "TransportCustomerMasterGoodCode",
      "TransportCustomerMasterGoodTitle",
      "TransportCustomerMasterGoodDescription",
      "TransportCustomerMasterGoodCustomerId"
    ],
  ...
  "Vi_DPA_TransportReqDeliverySummary": {
    "name": "Vi_DPA_TransportReqDeliverySummary",
    "source": "core",
    "columns": [
      "Id",
      "IsEditable",
  ...
  "Vi_DPA_TransportReqRide": {
    "name": "Vi_DPA_TransportReqRide",
    "source": "core",
    "columns": [
      "Id",
      "IsEditable",
  ...
  "Vi_DPA_TransportReqRidePassengersSuperDetail": {
    "name": "Vi_DPA_TransportReqRidePassengersSuperDetail",
    "source": "core",
    "columns": [
      "TransportReqRideId",
      "TransportReqRideIsEditable",
  ...
  "Vi_DPA_TransportReqRidesSuperDetail": {
    "name": "Vi_DPA_TransportReqRidesSuperDetail",
    "source": "core",
    "columns": [
      "DistributorId",
      "DistributorEntityId",
  ...
  "Vi_DPA_TransportTransact": {
    "name": "Vi_DPA_TransportTransact",
    "source": "core",
    "columns": [
      "Id",
      "FMS_OvId",
  ...
  "Vi_DPA_ZoneBase": {
    ...
    "definition": "select `BaseTable`.`Id` AS `Id`,`BaseTable`.`Code` AS `Code`,`BaseTable`.`Title` AS `Title`,`BaseTable`.`Description` AS `Description`,`BaseTable`.`ZoneType` AS `ZoneType` from `_ZoneBases` `BaseTable` where ((0 <> `BaseTable`.`RecordDeleted`) is not true)"
  },
  "Vi_DPA_ZoneGeofencesSuperDetail": {
    "name": "Vi_DPA_ZoneGeofencesSuperDetail",
    "source": "core",
    "columns": [
      "TransportGeoFenceBaseId",
      "TransportGeoFenceBaseCode",
      "TransportGeoFenceBaseTitle",
      "TransportGeoFenceBaseCoordinates",
      "ZoneId",
      "ZoneBaseCode",
      "ZoneBaseTitle",
      "ZoneBaseDescription",
  ...
  "Vi_DPC_BatchReqRidesDetail": {
    "name": "Vi_DPC_BatchReqRidesDetail",
    "source": "core",
    "columns": [
      "BatchId",
      "ItemOrder",
      ...
      "FromContactLocationId",
      "ToContactLocationId",
      "TransportRequestStatusENUM",
      "TransportRequestStatusTitle",
      "ContactId",
      "ServiceCategory",
      "ServiceClass",
      "VehicleType",
  ...
  "Vi_DPC_ContactLocations": {
    "name": "Vi_DPC_ContactLocations",
    "source": "core",
    "columns": [
      "Id",
      "FavAdrLabel",
  ...
  "Vi_DPC_ReqRideDestinationContactLocation": {
    "name": "Vi_DPC_ReqRideDestinationContactLocation",
    "source": "core",
    "columns": [
      "Id",
      "ContactId",
  ...
  "Vi_DPC_TransportAddons": {
    "name": "Vi_DPC_TransportAddons",
    "source": "core",
    "columns": [
      "Id",
      "CalculationItemTitle",
      "CalculationUnitTitle",
      "CalculateMethodTitle",
      "UnitAmount",
      "Price",
      "DistributorId",
      "CVOId",
  ...
  "Vi_DPC_TransportContactRegistryLastRecord": {
    "name": "Vi_DPC_TransportContactRegistryLastRecord",
    "source": "core",
    "columns": [
      "Id",
      "PossibleUserId",
  ...
  "Vi_DPC_TransportReqRideSummary": {
    ...
    "columns": [
      ...
      "CVOTitle",
      "FromContactLocationId",
      "FromContactLocationFavAdrLabel",
      "FromContactLocationTitle",
      "FromContactLocationLatitude",
      "FromContactLocationLongitude",
      "FromContactLocationAddress",
      "FromContactLocationCity",
      "FromContactLocationProvince",
      "FromContactLocationPostalCode",
      "FromContactLocationPlaceId",
      "ToContactLocationId",
      "ToContactLocationFavAdrLabel",
      "ToContactLocationTitle",
      "ToContactLocationLatitude",
      "ToContactLocationLongitude",
      "ToContactLocationAddress",
      "ToContactLocationCity",
      "ToContactLocationProvince",
      "ToContactLocationPostalCode",
      "ToContactLocationPlaceId",
      "PickupAfterDateTime",
      "PickupBeforeDateTime",
      "DropOffAfterDateTime",
      "DropOffBeforeDateTime",
